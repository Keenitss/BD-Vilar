...
-- INNER JOIN: Essa consulta busca registros que têm correspondências em ambas as tabelas.
...
-- Essa consulta retorna os nomes, ID_prato e ID_bebida da tabela Ingredientes, onde há correspondência na tabela Pedidos com base no ID_prato.

-- LEFT JOIN: Essa consulta traz todos os registros da tabela Ingredientes (esquerda) e os registros correspondentes da tabela Pedidos (direita). Se não houver correspondência, o resultado será NULL na tabela da direita.
...
-- A consulta traz todas as linhas da tabela Ingredientes e as correspondentes da tabela Pedidos. Quando não há correspondência em Pedidos, a consulta retorna NULL nas colunas referentes a Pedidos.

-- RIGHT JOIN: A consulta traz todos os registros da tabela Pedidos (direita) e os registros correspondentes da tabela Ingredientes (esquerda). Se não houver correspondência, o resultado será NULL nas colunas da esquerda.
...
-- Essa consulta retorna todas as linhas da tabela Pedidos, e as correspondências na tabela Ingredientes. Onde não houver correspondência, as colunas da tabela Ingredientes serão NULL.

-- CROSS JOIN: Essa consulta retorna o produto cartesiano das duas tabelas, ou seja, combina todas as linhas de Ingredientes com todas as linhas de Pedidos.

SELECT ingredientes.nome, ingredientes.ID_prato, ingredientes.ID_bebida
FROM Ingredientes
CROSS JOIN Pedidos
WHERE Ingredientes.ID_prato = Pedidos.ID_prato;

-- A consulta retorna todas as combinações possíveis de linhas entre Ingredientes e Pedidos, mas não exige uma condição para realizar essa junção.
